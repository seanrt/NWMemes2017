...
class TweetData():
    def __init__(self, tweet, url, replyCount, retweetCount, likeCount):
        self.tweet = tweet
        self.url = url
...
def getTweetDataFromSoup(soup):
    ...
    for div in soup.findAll('div', class_='content'):
        ...
        tweetData.append(TweetData(tweet, url, replyCount, retweetCount, likeCount))
    return tweetData
...
def updateTweetsInDatabase():
    ...
    for city in cities:
        ...
        for tweet in tweetData:
            print(tweet)
            databaseCursor.execute("INSERT INTO nwmeme.tweets (cityId, tweet, imageUrl, retweetCount, likesCount, repliesCount, tweetedTime) SELECT city.cityId, '"+tweet.tweet+"', '"+tweet.url+"', "+tweet.retweetCount+", "+tweet.likeCount+", "+tweet.replyCount+", 'a' FROM nwmeme.cities city WHERE cityName = '"+city+"'")
